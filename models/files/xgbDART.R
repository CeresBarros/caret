...
modelInfo <- list(label = "eXtreme Gradient Boosting",
                  ...
                  predict = function(modelFit, newdata, submodels = NULL) {
                    ...
                    out <- predict(modelFit, newdata, iterationrange = c(modelFit$niter, modelFit$niter))
                    ...
                    if(!is.null(submodels)) {
                      ...
                      for(j in seq(along = submodels$nrounds)) {
                        tmp_pred <- predict(modelFit, newdata, iterationrange = c(submodels$nrounds[j], submodels$nrounds[j]))
                        if(modelFit$problemType == "Classification") {
                          if(length(modelFit$obsLevels) == 2) {
                  ...
                  prob = function(modelFit, newdata, submodels = NULL) {
                    ...
                    if( !is.null(modelFit$param$objective) && modelFit$param$objective == 'binary:logitraw'){
                      p <- predict(modelFit, newdata, iterationrange = c(modelFit$niter, modelFit$niter))
